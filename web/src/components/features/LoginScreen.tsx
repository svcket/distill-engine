...
export default function LoginScreen() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6 font-sans selection:bg-white/20">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-10 group cursor-default">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-white/5 border border-white/10 mb-4 group-hover:border-white/20 transition-all duration-500">
        ...
        <div className="bg-white/[0.02] backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl overflow-hidden relative group">
          ...
          {!isSent ? (
            <div className="space-y-6 animate-in fade-in duration-700">
              <form onSubmit={handleEmailLogin} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full h-12 bg-white text-black font-medium text-sm rounded-xl hover:bg-white/90 active:scale-[0.98] transition-all flex items-center justify-center gap-2 group/btn disabled:opacity-50 disabled:active:scale-100"
                >
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-[0.2em]">
                  <span className="bg-[#0a0a0a]/50 px-2 text-white/20 backdrop-blur-sm">OR</span>
                </div>
              </div>

              <button
                onClick={handleGoogleLogin}
                className="w-full h-12 bg-white/5 border border-white/10 rounded-xl text-sm text-white hover:bg-white/10 active:scale-[0.98] transition-all flex items-center justify-center gap-3"
              >
            ...
            <div className="py-8 text-center animate-in zoom-in-95 fade-in duration-700">
              ...
              <h2 className="text-xl font-medium text-white mb-2">Verify your identity</h2>
              <p className="text-white/40 text-sm leading-relaxed max-w-[240px] mx-auto font-light">
                A verification link has been sent to <span className="text-white/80 font-normal">{email}</span>. Click it to authorize your session.
              </p>
              <button 
                onClick={() => setIsSent(false)}
                className="mt-8 text-xs text-white/30 hover:text-white/60 transition-colors uppercase tracking-widest underline underline-offset-4"
              >
